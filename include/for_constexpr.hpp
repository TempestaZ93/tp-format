...
struct static_for_impl {
  ...
  private:
    ...
    template <size_t flat_start, size_t flat_end, class flat_functor> struct flat_for {
      ...
      private:
        // Loop termination
        static inline void flat_next(std::integral_constant<size_t, flat_end + 1>,
                                     functor_types &&...) {}

        // Loop function that calls the function passed to it, as well as recurses
        template <size_t index>
        static inline void flat_next(std::integral_constant<size_t, index>,
    ...
    // It will recurse to more finer grained point until the points are no bigger than
    // sequence_width
    ...
    static inline void next(std::true_type, functor_types &&...functor_args) {
    ...
    template <typename sequence>
    static inline void next(std::false_type, functor_types &&...functor_args) {
